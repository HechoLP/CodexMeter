...
[![macOS Release](https://img.shields.io/badge/macOS-v0.1.5%20preview-6e5aff?style=flat-square)](Documentation/ReleaseNotes/0.1.5.md)
...
CodexMeter v0.1.5 is available from the public [CodexMeter Releases repository](https://github.com/HechoLP/CodexMeter-Releases/releases/tag/v0.1.5) as a certificate-free Universal 2 DMG and ZIP. The app uses an ad-hoc signature rather than an Apple Developer ID certificate, so macOS will not trust the first launch automatically. Verify the downloaded DMG and follow the one-time first-run steps below.
